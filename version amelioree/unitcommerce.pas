  ...
  if quoi=1 then
     ...
     prix :=10*cb;
        if choix=1 then begin //mise a jour de la bourse et de l'inventaire suite à l'achat
           setGold(getGold() - prix);
           setBois(getBois()+cb);
           if getBois() > getrsv() then begin //vérification que l'inventaire ne soit pas pleins suite à l'action d'acheter, sinon l'action est annulé
              setBois(getBois()-cb);
              setGold(getGold() + prix);
     ...
     prix :=10*cb;
        if choix=1 then begin //mise a jour de la bourse et de l'inventaire suite à l'achat
           ...
           if getTools() > getrsv() then begin //vérification que l'inventaire ne soit pas pleins suite à l'action d'acheter, sinon l'action est annulé
              setTools(getTools()-cb);
              setGold(getGold() + prix);
              writeln ('Ton inventaire est plein');
           end;
     ...
     prix :=7*cb;
          if choix=1 then begin //mise a jour de la bourse et de l'inventaire suite à l'achat
             ...
             if getLaine() > getrsv() then begin //vérification que l'inventaire ne soit pas pleins suite à l'action d'acheter, sinon l'action est annulé
                setLaine(getLaine - cb);
                setGold(getGold() + prix);
                writeln ('Ton inventaire est plein');
           end;
     ...
     prix :=9*cb;
        if choix=1 then begin //mise a jour de la bourse et de l'inventaire suite à l'achat
           ...
           if getTissu > getrsv() then begin //vérification que l'inventaire ne soit pas pleins suite à l'action d'acheter, sinon l'action est annulé
              setTissu(getTissu() - cb);
              setGold(getGold() + prix);
              writeln ('Ton inventaire est plein');
           end;
