...
begin
  ...
  readln(choix);                           //acheter ou vendre ?
  ...
  if quoi=1 then
     ...
     prix :=5*cb;
        if choix=1 then begin //mise a jour de la bourse et de l'inventaire suite à l'achat
           ...
           setFish(getFish()+cb);
           if getFish() > 40 then begin //vérification que l'inventaire ne soit pas pleins suite à l'action d'acheter, sinon l'action est annulé
              setFish(getFish()-cb);
              setGold(getGold()+ prix);
